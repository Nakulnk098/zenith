...
router.post('/', (req, res) => {
  ...
  const id = run('INSERT INTO habits (name, icon, color) VALUES (?, ?, ?)', [name, icon || '✨', color || '#6366f1']);
  // Fallback: if last_insert_rowid didn't work, find by MAX(id)
  let habit = get('SELECT * FROM habits WHERE id = ?', [id]);
  if (!habit) {
    habit = get('SELECT * FROM habits ORDER BY id DESC LIMIT 1');
  }
  if (habit) {
...
router.get('/:id/history', (req, res) => {
  ...
  const logs = all('SELECT date FROM habit_logs WHERE habit_id = ? AND date >= ? AND completed = 1 ORDER BY date ASC', [parseInt(req.params.id), startStr]);
